...
\begin{abstract}
Nonlinear optics is a field of research concerned with the effects that arise when electric fields in materials are large enough that the polarisation is no longer directly proportional to the electric field. The related processes usually require a large interaction volume and a high laser power, but nanoantennas are an emerging method of bringing these effects to a smaller scale, with hopes that this allows for eventual easier integration in optical devices. This review presents an introduction to the above concepts, a few examples of research into optimising the output of nonlinear generation, and a method of computationally predicting this output in order to inform experimental investigation.
\end{abstract}
...
An example of a FDTD solver is the Lumerical 3D Electromagnetic Simulator \cite{lumericalinc.} -- this is the piece of software we are most likely going to use throughout our project.
...
We start with Lorentz reciprocity, which is the principle stating that exchanging the source and detector of an electric field will not result in a change of the reading. This can be written as an integral over two volumes:
\begin{equation}
	\label{eq:lorentz-rec}
	\int_A\mathbf{j}_A(\mathbf{r}_A)\cdot\mathbf{E}_D(\mathbf{r}_A)dV_A
	= \int_D\mathbf{j}_D(\mathbf{r}_D)\cdot\mathbf{E}_A(\mathbf{r}_D)dV_D,
\end{equation}
where $\mathbf{j}_A$ is the nonlinear contribution to the current density in the antenna induced by the pump, which induces an electric field $\mathbf{E}_A$, and $\mathbf{j}_A$ is a dummy current dipole placed at the detector, which induces a field $\mathbf{E}_D$.

By realising that
\begin{equation}
	\label{eq:lorentz-p}
	\mathbf{j}_A(\mathbf{r})=\frac{\partial\mathbf{P}_A(\mathbf{r})}{\partial t}
	=i\omega\mathbf{P}_A(\mathbf{r})
\end{equation}
for a polarisation excited by an electromagnetic wave (the $\mathbf{P}_A$ here is the nonlinear polarisation of a chosen order, with $\omega$ being the signal frequency), and writing the electric dipole as
\begin{equation}
	\label{eq:lorentz-dipole}
	\mathbf{j}_D(\mathbf{r}) = J_0\Delta L\cdot\delta(\mathbf{r}-\mathbf{r}_{D0})\hat{\mathbf{j}}e^{i\omega t},
\end{equation}
where $\hat{\mathbf{j}}$ is the polarisation axis, $\delta$ the three-dimensional Dirac delta function, $\Delta L$ the dipole's length, and $\mathbf{r}_{D0}$ its location, we can integrate \eqref{eq:lorentz-rec} to give
\begin{equation}
	\label{eq:lorentz-final}
	\mathbf{E}_A(\mathbf{r}_{D0})\cdot\hat{\mathbf{j}}=
	\frac{i\omega e^{-i\omega t}}{J_0\Delta L}
	\int_A\mathbf{P}_A(\mathbf{r}_A)\cdot\mathbf{E}_D(\mathbf{r}_A)dV_A,
\end{equation}
which is the electric field signal the detector will see.

Now in order to compute the overlap integral in \eqref{eq:lorentz-final}, we need to know the nonlinear polarisation, and $\mathbf{E}_D$ at the nanoantenna. These can be obtained using two separate FDTD simulations of the antenna, one for the pump, and one for a dummy field (sometimes approximated as a plane wave) coming from a dipole at the detector.

\subsection{Methods of optimising the antenna signal}
Fig.~\ref{fig:steps} schematically shows the many steps that contribute to the final efficiency of nonlinear generation from an antenna. The pump must first couple into the antenna (the efficiency of this depends on the far-field representation of the mode), the mode can be more or less resonant (resulting in various levels of field enhancement), the overlap between the pump and signal modes we're exciting has to be good (see \eqref{eq:lorentz-final}), the signal mode, should can be further enhanced by resonance, and it should have a strong far-field presence.
...
%TODO also write the abstract, that's important!
